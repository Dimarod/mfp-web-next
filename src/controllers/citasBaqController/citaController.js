import { CitaService } from "@/services/citasBaqServices/citaService";
...
export const citaController = {
  ...
  consultarSobrecupo: async (req, res) => {
    try {
      ...
      const haySobrecupo = await CitaService.verificarSobrecupo(fecha, horab);
  ...
  checkAvailability: async (req, res) => {
    try {
      const { fecha, tipoBaq } = req.query

      if (!fecha || !tipoBaq) {
        return res.status(400).json({ message: "Faltan datos" });
      }

      const blocked = await CitaService.obtenerDisponibilidad(fecha, tipoBaq);
