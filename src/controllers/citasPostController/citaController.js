import { CitaService } from "@/services/citasPostServices/citaService";
...
export const citaController = {
  ...
  consultarSobrecupo: async (req, res) => {
    try {
      const { fecha, horapc } = req.body;

      const haySobrecupo = await CitaService.verificarSobrecupo(fecha, horapc);
  ...
  checkAvailability: async (req, res) => {
        try {
          const {fecha, tipoPostCorp} = req.query
    
          if (!fecha || !tipoPostCorp) {
            return res.status(400).json({ message: "Faltan datos" });
          }
    
          const blocked = await CitaService.obtenerDisponibilidad(fecha, tipoPostCorp);
