...
const citasCorpBaq = ({ citas: citasIniciales }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`/api/citasBaq?id=${id}`);
      if (search) {
        const res = await axios.get('/api/citasBaq/', { params: { fecha: search } })
  ...
  const handleSubmit = async (e) => {
    ...
    if (!search) return
    try {
      const resultado = await axios.get('/api/citasBaq', { params: { fecha: search } });
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-transparent shadow-xl border border-gray-50 rounded-md py-6 md:py-10 px-4 mt-24 md:mt-12 mb-12">
        ...
        <div className="max-w-7xl mx-auto mb-6 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
          <div>
            <h1 className="font-display text-2xl md:text-3xl text-brand-black">Agenda Barranquilla</h1>
            <p className="font-sans text-sm text-gray-500 mt-1">Gestión de citas Cosmoterapia</p>
          </div>
          
          {/* Barra de Búsqueda Adaptable */}
          <form 
            className="flex w-full md:w-auto items-center bg-white p-1 rounded-full shadow-sm border border-gray-200"
          ...
          >
            ...
            <input
              ...
              type="date"
              className="bg-transparent border-none outline-none text-gray-700 text-sm py-2 px-2 font-sans w-full md:w-auto"
            />
            <button
              type="submit"
              className="bg-brand-black text-white rounded-full px-4 md:px-6 py-2 text-sm font-medium hover:bg-gray-800 transition-colors ml-2"
            >
              Buscar
            </button>
            {search && (
              <button type="button" onClick={clearSearch} className="text-gray-400 hover:text-red-500 px-3">
              ...
              <div key={cita._id} className="bg-white rounded-2xl p-5 shadow-md border border-gray-100 flex flex-col gap-3">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <span className="inline-block mt-1 px-2 py-0.5 rounded-md text-xs font-medium bg-brand-vanilla/10 text-brand-vanilla-dark border border-brand-vanilla/20">
                      {cita.tipoBaq}
                    </span>
                  </div>
                  <button 
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mt-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {HORARIOS[cita.horab] || "N/A"}
                  </div>
                </div>

                {/* Footer Tarjeta: Teléfono */}
                <div className="pt-3 border-t border-gray-100">
                  <a href={`tel:${cita.telefono}`} className="flex items-center justify-center gap-2 w-full py-2 bg-gray-50 rounded-lg text-brand-black font-medium hover:bg-brand-vanilla hover:text-white transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                    Llamar: {cita.telefono}
                  </a>
                </div>
              </div>
            ))
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {dataToShow.length > 0 ? (
                  dataToShow.map((cita) => (
                    <Fragment key={cita._id}>
                      <tr className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-6 py-4"><span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-brand-vanilla/10 text-brand-vanilla-dark border border-brand-vanilla/20">{cita.tipoBaq}</span></td>
                        <td className="px-6 py-4"><span className="font-sans text-sm font-semibold text-gray-700 bg-gray-100 px-2 py-1 rounded">{HORARIOS[cita.horab] || "N/A"}</span></td>
                  ...
                  <tr><td colSpan="6" className="px-6 py-12 text-center text-gray-400">No se encontraron citas registradas.</td></tr>
                )}
              </tbody>
            </table>
          </div>
