...
const citasPostBaq = ({ citas: citasIniciales }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`/api/citasPostBaq?id=${id}`);
      if (search) {
        const res = await axios.get('/api/citasPostBaq/', { params: { fecha: search } })
  ...
  const handleSubmit = async (e) => {
    ...
    if (!search) return
    try {
      const resultado = await axios.get('/api/citasPostBaq', { params: { fecha: search } });
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-transparent shadow-xl rounded-lg border border-gray-50 py-6 md:py-10 px-4 mt-24 md:mt-12 mb-6">
        ...
        <div className="max-w-7xl mx-auto mb-6 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
          <div>
            <h1 className="font-display text-2xl md:text-3xl text-brand-black">Agenda Postoperatoria</h1>
            <p className="font-sans text-sm text-gray-500 mt-1">Gestión de citas - Sede Postoperatoria Adrián Ospino</p>
          </div>
              ...
              <div key={cita._id} className="bg-white rounded-2xl p-5 shadow-md border border-gray-100 flex flex-col gap-3">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    {/* Badge con color diferente para postoperatorio */}
                    <span className="inline-block mt-1 px-2 py-0.5 rounded-md text-xs font-medium bg-[#8B5E3C]/10 text-[#8B5E3C] border border-[#8B5E3C]/20">{cita.tipoPostCorp}</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mt-2">
                  ...
                  <div className="flex items-center gap-2"><svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{HORARIOS[cita.horapc] || "N/A"}</div>
                </div>
                  ...
                  dataToShow.map((cita) => (
                    <Fragment key={cita._id}>
                      <tr className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-6 py-4"><span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-[#8B5E3C]/10 text-[#8B5E3C] border border-[#8B5E3C]/20">{cita.tipoPostCorp}</span></td>
                        <td className="px-6 py-4"><span className="font-sans text-sm font-semibold text-gray-700 bg-gray-100 px-2 py-1 rounded">{HORARIOS[cita.horapc] || "N/A"}</span></td>
